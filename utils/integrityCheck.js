...
function execute(event, callback) {
    ...
    util.checkArgs([{
        ...
        .then(res => endRequest(res, event, callback))
        .catch(err => endRequestError(err, event, callback))
}
...
function endRequest(res, event, callback) {
    ...
    logMessage(`INTEGRITY CHECK FINISHED - ${event.table.name}`, 1);
    
    callback(null, logString);
}

function endRequestError(err, event, callback) {
    logMessage(`${err.message || JSON.stringify(err)}`, 1);
    logMessage(`INTEGRITY CHECK FINISHED ${event.table.name || ''}`, 1);
